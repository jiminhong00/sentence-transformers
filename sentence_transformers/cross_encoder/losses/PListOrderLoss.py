...
class PListOrderLambdaWeight(nn.Module):
...
class PListOrderLoss(nn.Module):
    def __init__(
        self,
        model: CrossEncoder,
        lambda_weight: PListOrderLambdaWeight | None = PListOrderLambdaWeight(),
    ...
    def forward(self, inputs: list[list[str], list[list[str]]], labels: list[Tensor]) -> Tensor:
        ...
        sep_tok = self.model.tokenizer.sep_token
        sp_tok = ["<s>", "<p1>", "<p2>", "<p3>"]
        sp_tok_id = self.model.tokenizer.convert_tokens_to_ids(sp_tok)
        ...
        for i in range(0, len(inputs), mini_batch_size):
            ...
            mini_batch = inputs[i : i + mini_batch_size]
            # mini_batch_pairs = pairs[i : i + mini_batch_size]
            formatted_mini_batch = [f"{q}{sep_tok}<p1>{p1}{sep_tok}<p2>{p2}{sep_tok}<p3>{p3}" for q, p1, p2, p3 in mini_batch]
            tokens = self.model.tokenizer(
                formatted_mini_batch,
                padding=True,
                truncation=True,
                return_tensors="pt",
            )
