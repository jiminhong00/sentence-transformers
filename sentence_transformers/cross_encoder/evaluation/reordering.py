...
class CrossEncoderReorderingEvaluator(SentenceEvaluator):
    """
    ...
    def __init__(
        ...
        self.csv_file = "CrossEncoderReorderingEvaluator" + ("_" + name if name else "") + f".csv"
        # self.csv_file = "CrossEncoderReorderingEvaluator" + ("_" + name if name else "") + f"_results_@{self.at_k}.csv"
        # self.csv_headers = ["epoch", "steps", "MAP", f"MRR@{self.at_k}", f"NDCG@{self.at_k}"]
        self.csv_headers = ["epoch", "steps", "ALL_ACC", f"ALL_1ST", f"ALL_CLS", f"MEAN_SCORE", f"MEAN_WEIGHT"]
        self.write_csv = write_csv
        # self.primary_metric = f"ndcg@{self.at_k}"
        self.primary_metrix = f"all_acc"
    ...
    def __call__(
        ...
        logger.info(f"CrossEncoderReorderingEvaluator: Evaluating the model on the {self.name} dataset{out_txt}:")
        ...
        for instance in tqdm(self.samples, desc="Evaluating samples", disable=not self.show_progress_bar, leave=False):
            ...
            model_input = [query, p1, p2, p3]
            pred_scores = model.predict(model_input, convert_to_numpy=True, show_progress_bar=False)[0]
            # print("pred scores:", pred_scores)
            # print("pred scores:",  [score for score in pred_scores if score.size > 0])
            # print("labels:", labels)
            # pred_rank = len(pred_scores) - rankdata(pred_scores, method="ordinal") + 1
            # label_rank = len(labels) - rankdata(labels, method="ordinal") + 1
            pred_sort = sorted(range(len(pred_scores)), key=lambda i: pred_scores[i], reverse=True)
            label_sort = sorted(range(len(labels)), key=lambda i: labels[i], reverse=True)
            # print("pred_sort:", pred_sort)
            # print("label_sort", label_sort)
            ...
            for i in range(len(pred_sort)):
                if pred_sort[i] == label_sort[i]:
                    right_num += 1
                    right_weight += 0.1 * (4 - i)
            
            rank_all = rank_all + int(flag)
            rank_1st = rank_1st + int(pred_sort[0] == label_sort[0])
            rank_cls = rank_cls + int(pred_sort.index(0) == label_sort.index(0))
            score_all = right_num / len(pred_sort)
            score_weight =  right_weight
            
            # all_mrr_scores.append(mrr)
            # all_ndcg_scores.append(ndcg)
            # all_ap_scores.append(ap)

        mean_all = rank_all / len(self.samples)
        mean_1st = rank_1st / len(self.samples)
        mean_cls = rank_cls / len(self.samples)
        mean_score = score_all / len(self.samples)
        mean_weight = score_weight / len(self.samples)
        ...
        logger.info(
            ...
            # f"Negatives: Min {np.min(num_negatives):.1f}, Mean {np.mean(num_negatives):.1f}, Max {np.max(num_negatives
            # ):.1f}"
        )
        logger.info(f"ALL_ACC: {mean_all * 100:.2f}")
        logger.info(f"1ST_ACC: {mean_1st * 100:.2f}")
        logger.info(f"CLS_ACC: {mean_cls * 100:.2f}")
        logger.info(f"MEAN_SCORE: {mean_score * 100:.2f}")
        logger.info(f"MEAN_WEIGHT: {mean_weight * 100:.2f}")
        ...
        if output_path is not None and self.write_csv:
            ...
            with open(csv_path, mode="a" if output_file_exists else "w", encoding="utf-8") as f:
                ...
                writer.writerow([epoch, steps, mean_all, mean_1st, mean_cls])
